...
namespace Chem4Word.Libraries.Database
{
    public class Library
    {
        ...
        public Library(TelemetryWriter telemetry, LibrarySettings settings)
        {
            ...
            string libraryTarget = Path.Combine(_settings.ProgramDataPath, Constants.LibraryFileName);
            ...
            if (!File.Exists(libraryTarget))
            {
                ...
                if (stream != null)
                {
                    using (ZipFile zip = ZipFile.Read(stream))
                    {
                        ...
                        zip.ExtractAll(_settings.ProgramDataPath, ExtractExistingFileAction.OverwriteSilently);
                    }
                }
            }
        }
        ...
        {
            string path = Path.Combine(_settings.ProgramDataPath, Constants.LibraryFileName);
        ...
        public Dictionary<string, int> GetLibraryNames()
        {
            ...
            {
                using (var form = new ReportError(_telemetry, _settings.ParentTopLeft, module, ex))
                {
                    form.ShowDialog();
                }
            }
        ...
        public bool ImportCml(string cmlFile, SQLiteTransaction transaction, bool calculateProperties = false)
        {
            ...
            try
            {
                ...
                if (model != null)
                {
                    var outcome = model.EnsureBondLength(_settings.PreferredBondLength, _settings.SetBondLengthOnImport);
                    if (_settings.RemoveExplicitHydrogensOnImport)
                    {
                        ...
                        || model.TotalBondsCount > 0 && model.MeanBondLength > 0)
                    {
                        if (calculateProperties)
                        {
                            ...
                            var pc = new WebServices.PropertyCalculator(_telemetry, _settings.ParentTopLeft, _settings.Version);
        ...
        public SQLiteDataReader GetChemistryByTags(SQLiteConnection conn)
        {
            string module = $"{_product}.{_class}.{MethodBase.GetCurrentMethod().Name}()";
            try
            {
